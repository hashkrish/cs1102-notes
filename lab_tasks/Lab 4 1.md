...
Regular expressions (regex) are patterns used to match character combinations in strings. They are widely used in text processing for searching, matching, and replacing patterns in text data. The regular expression forms backbone for textual processing using shell scripting. The utilities grep, sed and awk relies on the regular expression to process textual data.
...
4. **Recursive Search:**
   - Task: Search for a pattern recursively in all files within a directory. (you will need to create directory structure with some content in files in the directory structure. You can use the Lab 3 `for` loop activity to do the task.)
...
For the following do the task and create one more similar task there by compiling at least 2 examples/usecases for each of the sub-titles.

Create the input test cases as required. task should accompany with additional similar task you perform using your own example.
...
9. Report one use case for each of the in-built variable and function in the introduction of the activity.
